...
| `src/i18n.rs` | The message catalogue both front ends look strings up in. English is compiled in and is the fallback for every other language, so a partial translation is a useful one; other languages are TOML files under `lang/` beside the config, which means adding one needs no compiler. Keys are dotted paths naming *where* a string appears, and `every_key_the_code_uses_is_in_the_catalogue` fails the build for one the catalogue lacks. |
